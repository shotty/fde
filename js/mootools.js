//browser management
window.safari = ( window.navigator.userAgent.indexOf('Safari')>-1 );
window.ie = ( window.navigator.appName.indexOf('Explorer')>-1 );
window.firefox = (  !window.safari && !window.ie );


if ( window.ie ){
	if ( window.navigator.appVersion.indexOf('MSIE')>-1 ){
		var app = window.navigator.appVersion.substring( 
			window.navigator.appVersion.indexOf('MSIE')+1
		);
		var version = app.substring(0,app.indexOf(';'));
		window[ (version.indexOf('7.')>-1 ) ? 'ie7':'ie6'] = true;
	}
}
/*
Script: Core.js
	...
	fireEvent: function(type, args, delay){
		...
		if (this.$events && this.$events[type]){
			this.$events[type].each(function(fn){
				var rsp = fn.create({'bind': this, 'delay': delay, 'arguments': args})();
				result = ( rsp != undefined ) ? rsp : result;
				if ( result == false)
					return Array.SKIP_EACH;
			}, this);
		}
		return result;
	},
	...
	*/
	each:function(fn,bind){
		for (var i = 0, j = this.length; i < j; i++){
			if ( fn.call(bind, this[i], i, this) == Array.SKIP_EACH )
				return;
		}
	},
	...
	forEach: function(fn, bind){
		for (var i = 0, j = this.length; i < j; i++){
			if ( fn.call(bind, this[i], i, this) == Array.SKIP_PAGE )
				return;
		}
	},
...
Element.extend({
	...
	*/
	...
	setHTML: function(){
		...
		//if ( this.getTag() == 'input' ) this.setProperty('value','');
		return this;
	},
...
Drag.Base = new Class({
	...
	start: function(event){
		var rsp = this.fireEvent('onBeforeStart', this.element);
		if ( rsp == false )
			return;
...
Element.extend({
	...
	*/
	...
	send: function(options,url){
		return new Ajax(  (this.getProperty('action') || url ), $merge({data: this.toQueryString()}, options, {method: 'post'})).request();
	},
	
	sendWithJson: function(options,url){
		return new Json.Remote( (this.getProperty('action')||url)  , $merge(options, {method: 'post'})).send(this.toQueryString());
	}
